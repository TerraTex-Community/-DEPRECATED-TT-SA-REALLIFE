			...
			for theKey, theMarker in ipairs(tuningMarker) do
				if(theMarker[1]==source)then
					if(isGarageOpen(theMarker[2]) and not(torData[theMarker[2]]["disabled"]))then
						...
						if(getVehicleType(hitElement)=="Automobile" and privCars[hitElement]==true )then
                            if(string.lower(vioGetElementData(hitElement,"besitzer"))==string.lower(getPlayerName(driver)))then
